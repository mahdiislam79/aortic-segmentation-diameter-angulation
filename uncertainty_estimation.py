...
# ----- SETTINGS -----
source_folder = '/gpfs/data/fs71894/mahdi_i/ActiveLearning/dataset/nnunet_data/inference/Dataset_011/pred_proba'
# filtered_folder = '/gpfs/data/fs71894/mahdi_i/nnunet_accessroute/inference/inference_stage1/proba'
output_path = '/gpfs/data/fs71894/mahdi_i/ActiveLearning/dataset/nnunet_data/inference/Dataset_011/uncertainty_estimations/uncertainty_summary.csv'
    ...
    try:
        ...
        for i in range(foreground_probs.shape[0]):
            ...
            entropy = -(np.clip(probs_i, 1e-6, 1 - 1e-6) * np.log(np.clip(probs_i, 1e-6, 1 - 1e-6)) + 
                        (1 - probs_i) * np.log(np.clip(1 - probs_i, 1e-6, 1 - 1e-6))) # formula = -p*log(p) - (1-p)*log(1-p)
